...
{
	if (cvar == hDamage) NadeDamage = StringToInt(newVal);
	
	else if (cvar == hRadius) NadeRadius = StringToInt(newVal);
	...
	else if (cvar == hType) NadeType = StringToInt(newVal);
	
	else if (cvar == hEnable) IsEnabled = StringToInt(newVal);
	...
	else if (cvar == hTeam) NadeAllowTeam = GetConVarInt(hTeam) + 1;
	
	else if (cvar == hArc) ArcEnabled = StringToInt(newVal);
	
	else if (cvar == hAmmo) NadeAmmo = StringToInt(newVal);
	
	else if (cvar == hPrice) NadePrice = StringToInt(newVal);
}
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((NadeAllowTeam != 1) && (NadeAllowTeam != ClientTeam)) return Plugin_Continue;
	...
	if (IsEnabled)
	{
		if (!NadePrice)
		{
			...
			if (NadeAmmo) PrintToChat(client, "\x04[Homing Missiles]\x05 Missiles left = %i", MissilesLeft[client]);
		}
		else MissilesLeft[client] = 0;
	}
...
{
	if (NadePrice && IsEnabled) PrintToChatAll("\x04[Homing Missiles]\x05 Type !buymissile in chat to buy a missile for $%i.", NadePrice);
}
		...
		else
		{
			...
			else
			{
				...
				if (StrEqual("hegrenade_projectile", ReplaceNade, false))
				{
					...
					AmmoReserve = GetEntData(client, ArrayOffset);
...
public InitMissile(const String:output[], caller, activator, Float:delay)
{
	...
	if (NadeOwner == -1) return;
	
	//ignore the projectile if this team can't use missiles.
	new NadeTeam = GetEntProp(caller, Prop_Send, "m_iTeamNum");
	if ((NadeAllowTeam != 1) && (NadeAllowTeam != NadeTeam)) return;
	
	if (NadeAmmo)
	{
		if (!MissilesLeft[NadeOwner]) return;
		else
		{
			MissilesLeft[NadeOwner]--;
			PrintToChat(NadeOwner, "\x05 Missiles left = %i", MissilesLeft[NadeOwner]);
		}
	}
